...
This document provides guidelines for internationalization of the NIX Core software.
...
This adds `<numerusform>`s to the respective `.ts` file, which can be translated separately depending on the language. In English, this is simply:
    ...
    <message numerus="yes">
        <source>%n active connection(s) to NIX network</source>
        <translation>
            <numerusform>%n active connection to NIX network</numerusform>
            <numerusform>%n active connections to NIX network</numerusform>
        </translation>
    </message>
...
This can be checked by executing `make translate` in the `src` directory, then verifying that `nix_en.ts` remains unchanged.
